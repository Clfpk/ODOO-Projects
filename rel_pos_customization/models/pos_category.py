...
class PosCategory(models.Model):
    ...
    product_ids = fields.Many2many('product.product', compute='_compute_product_ids_update')
    ...
    def _unlink_except_session_open(self):
        if self.search_count([('id', 'in', self.ids)]):
            if self.env['pos.session'].sudo().search_count(
                    [('state', '!=', 'closed'), ('company_id', '=', self.env.company.id)]):
                raise UserError(_('You cannot delete a point of sale category while a session is still opened.'))

    def _compute_product_count(self):
        read_group_res = self.env['product.template']._read_group([('pos_categ_ids', 'child_of', self.ids)],
                                                                  ['pos_categ_ids'], ['__count'])
    ...
    @api.depends('product_ids.pos_categ_ids')
    def _compute_product_ids_update(self):
        for rec in self:
            products = self.env['product.product'].search([('pos_categ_ids', 'child_of', self.id)])
            if products:
...
class ReportSaleDetails(models.AbstractModel):
    ...
    def _get_products_and_taxes_dict(self, line, products, taxes, currency):
        key2 = (line.product_id, line.price_unit, line.discount, line.note)
        keys1 = line.product_id.product_tmpl_id.pos_categ_ids.mapped("display_name") or [_('Not Categorized')]
        for key1 in keys1:
        ...
        for category_name, product_list in products_sold.items():
            category_dictionnary = {
                ...
                'products': sorted([{
                    ...
                    'note': note
                } for (product, price_unit, discount, note), qty in product_list.items()],
