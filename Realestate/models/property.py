...
class RealEstateProperty(models.Model):
    ...
    #     invoice = self.env['account_changes.move'].create({
    #         'partner_id': self.buyer_id.id,  # Use the property's buyer as the customer
    #         'move_type': 'out_invoice',  # Customer Invoice
    #         'journal_id': self.env['account_changes.journal'].search([('type', '=', 'sale')], limit=1).id,
    #         # Use a suitable sales journal
    #     })
    #
    #     self.env['account_changes.move.line'].create({
    #         'name': 'Selling Price',
    #         'quantity': 1,
